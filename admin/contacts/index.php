...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['id']) && $pdo) {
    if (($_POST['csrf_token'] ?? '') !== $_SESSION['csrf_token']) {
        ...
        header('Location: /sydney-pest-removal/admin/contacts'); exit;
    }
    $id     = (int)$_POST['id'];
    $action = $_POST['action'];
    ...
    if ($action === 'mark_read') {
        ...
        header('Location: /sydney-pest-removal/admin/contacts?view=' . $id);
        exit;
    }
    ...
    if ($action === 'mark_unread') {
        ...
        header('Location: /sydney-pest-removal/admin/contacts');
        exit;
    }
...
<div class="max-w-5xl mx-auto">
    ...
    <!-- ── Single Message View ───────────────────────────────── -->
    <div class="mb-6">
        <a href="/sydney-pest-removal/admin/contacts" class="text-sm text-gray-400 hover:text-primary flex items-center gap-1 mb-4 w-fit">
    ...
    <form method="GET" class="bg-white rounded-xl shadow-sm p-4 mb-6 flex flex-wrap gap-3 items-end">
        ...
        <a href="/sydney-pest-removal/admin/contacts" class="text-sm text-gray-400 hover:text-gray-700 py-2">Clear</a>
        <?php endif; ?>
    </form>
