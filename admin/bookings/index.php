...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'], $_POST['id']) && $pdo) {
    if (($_POST['csrf_token'] ?? '') !== $_SESSION['csrf_token']) {
        ...
        header('Location: /admin/bookings'); exit;
    }
    $id     = (int)$_POST['id'];
    $action = $_POST['action'];
    ...
    if ($action === 'status' && isset($_POST['status']) && in_array($_POST['status'], $valid_statuses)) {
        ...
        header('Location: /admin/bookings' . $view_redirect);
        exit;
    }
...
<div class="max-w-6xl mx-auto">
    ...
    <div class="mb-6">
        <a href="/admin/bookings" class="text-sm text-gray-400 hover:text-primary flex items-center gap-1 mb-4 w-fit">
    ...
    <form method="GET" class="bg-white rounded-xl shadow-sm p-4 mb-6 flex flex-wrap gap-3 items-end">
        ...
        <a href="/admin/bookings" class="text-sm text-gray-400 hover:text-gray-700 py-2">Clear</a>
        <?php endif; ?>
    </form>
